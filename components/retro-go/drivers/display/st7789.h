...
static rg_task_t *spi_task_handle;
static volatile bool spi_task_running;
...
static void spi_task(void *arg)
{
    ...
    while (spi_task_running)
    {
        if (spi_device_get_trans_result(spi_dev, &t, pdMS_TO_TICKS(100)) == ESP_OK)
        {
            if ((int)t->user & 2)
...
static void spi_init(void)
{
    ...
    {
        void *trans = heap_caps_malloc(sizeof(spi_transaction_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
        xQueueSend(spi_transactions, &trans, portMAX_DELAY);
    }

    while (uxQueueSpacesAvailable(spi_buffers))
    {
        void *buffer = heap_caps_aligned_calloc(32, 1, SPI_BUFFER_LENGTH, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
        xQueueSend(spi_buffers, &buffer, portMAX_DELAY);
    }
    ...
    const spi_device_interface_config_t devcfg = {
        ...
        .cs_ena_posttrans = 0,
        .input_delay_ns = 0,
        #if defined(RG_TARGET_ESP32_P4) || defined(RG_TARGET_ESP32P4)
    ...
    spi_task_handle = rg_task_create("rg_spi", &spi_task, NULL, 1.5 * 1024, RG_TASK_PRIORITY_8, RG_PERF_CORE_1);
}
...
static void spi_deinit(void)
{
    ...
    spi_transaction_t *t;
    int pending = 0;
    while (spi_device_get_trans_result(spi_dev, &t, pdMS_TO_TICKS(100)) == ESP_OK)
    {
        pending++;
        if ((int)t->user & 2)
            spi_give_buffer((uint16_t *)t->tx_buffer);
    ...
    if (pending > 0)
        RG_LOGD("Cleared %d pending transactions", pending);

    esp_err_t ret = spi_bus_remove_device(spi_dev);
    if (ret != ESP_OK)
        RG_LOGE("spi_bus_remove_device failed: %s", esp_err_to_name(ret));

    ret = spi_bus_free(RG_SCREEN_HOST);
    if (ret != ESP_OK)
        RG_LOGE("spi_bus_free failed: %s", esp_err_to_name(ret));

    if (spi_transactions)
    {
        spi_transaction_t *t;
        while (xQueueReceive(spi_transactions, &t, 0) == pdTRUE)
        {
            free(t);
        }
        vQueueDelete(spi_transactions);
        spi_transactions = NULL;
    }

    if (spi_buffers)
    {
        uint16_t *buffer;
        while (xQueueReceive(spi_buffers, &buffer, 0) == pdTRUE)
        {
            free(buffer);
        }
        vQueueDelete(spi_buffers);
        spi_buffers = NULL;
    }
}
...
static void lcd_set_window(int left, int top, int width, int height)
{
    ...
    ST7789_CMD(0x2A, left >> 8, left & 0xff, right >> 8, right & 0xff); // Horiz
    ST7789_CMD(0x2B, top >> 8, top & 0xff, bottom >> 8, bottom & 0xff); // Vert
