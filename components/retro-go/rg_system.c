        ...
        if (statistics.lastTick < rg_system_timer() - app.tickTimeout)
        {
            ...
            if (rg_input_wait_for_key(RG_KEY_MENU, true, 1000))
            {
                ...
                if (!rg_input_wait_for_key(RG_KEY_MENU, false, 2000))
                    RG_PANIC(_("Application terminated!")); // We're not in a nice state, don't normal exit
            }
        }
...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
    ...
    tasks_mutex = rg_mutex_create();
    RG_ASSERT(tasks_mutex, "Failed to create tasks mutex");
    ...
    int lang_id = rg_settings_get_number(NS_GLOBAL, "Language", RG_LANG_DEFAULT);
    if (lang_id >= 0 && lang_id < RG_LANG_MAX)
        rg_localization_set_language_id(lang_id);
    ...
    else if (showCrashDialog)
    {
        ...
        char message[400];
        strcpy(message, _("Application crashed"));
        if (panicTrace.magicWord == RG_STRUCT_MAGIC)
        {
            ...
            {
                strcat(message, "\n");
                strcat(message, _("Log saved to SD Card."));
            }
        }
    ...
    {
        app.logLevel = saved_log_level;
        esp_log_level_t esp_level;
        switch (saved_log_level)
        {
            case RG_LOG_PRINTF:   esp_level = ESP_LOG_NONE; break;
            case RG_LOG_ERROR:    esp_level = ESP_LOG_ERROR; break;
            case RG_LOG_WARN:     esp_level = ESP_LOG_WARN; break;
            case RG_LOG_INFO:     esp_level = ESP_LOG_INFO; break;
            case RG_LOG_DEBUG:    esp_level = ESP_LOG_DEBUG; break;
            case RG_LOG_VERBOSE:  esp_level = ESP_LOG_VERBOSE; break;
    ...
    if (app.lowMemoryMode)
        rg_gui_alert(_("External memory not detected"), _("Boot will continue but it will surely crash..."));
...
static void task_wrapper(void *arg)
{
    ...
    rg_mutex_take(tasks_mutex, -1);
    memset(task, 0, sizeof(rg_task_t));
    rg_mutex_give(tasks_mutex);
    vTaskDelete(NULL);
}
...
void rg_system_set_log_level(rg_log_level_t level)
{
    ...
    {
        app.logLevel = level;
        rg_settings_set_number(NS_GLOBAL, SETTING_LOG_LEVEL, level);
        esp_log_level_t esp_level;
        switch (level)
        {
            case RG_LOG_PRINTF:   esp_level = ESP_LOG_NONE; break;
...
#define UNLOCK_PROFILE() rg_mutex_give(profile->lock)
