...
void rg_settings_init(bool _safe_mode)
{
    ...
    // In safe mode, try to load only the global language setting
    if (safe_mode)
    {
        char *data; size_t data_len;
        char pathbuf[RG_PATH_MAX];
        snprintf(pathbuf, RG_PATH_MAX, "%s/global.json", RG_BASE_PATH_CONFIG);
        if (rg_storage_read_file(pathbuf, (void **)&data, &data_len, 0))
        {
            cJSON *values = cJSON_Parse(data);
            if (!values)
            ...
            if (values)
            {
                cJSON *lang = cJSON_GetObjectItem(values, "Language");
                if (lang)
                {
                    cJSON *global_values = cJSON_GetObjectItem(cJSON_GetObjectItem(config_root, "global"), "values");
                    if (global_values)
                        cJSON_AddNumberToObject(global_values, "Language", cJSON_GetNumberValue(lang));
                }
                cJSON_Delete(values);
            }
            free(data);
