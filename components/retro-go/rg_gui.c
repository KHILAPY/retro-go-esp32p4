...
void rg_gui_init(void)
{
    ...
    int lang_id = rg_settings_get_number(NS_GLOBAL, SETTING_LANGUAGE, RG_LANG_DEFAULT);
    if (lang_id >= RG_LANG_MAX)
    {
        if (lang_id == 3)
            lang_id = RG_LANG_ZH;
        else
            lang_id = 0;
        rg_settings_set_number(NS_GLOBAL, SETTING_LANGUAGE, lang_id);
    }
...
bool rg_gui_set_language_id(int index)
{
    if (rg_localization_set_language_id(index))
    {
        ...
        // Auto-switch to Chinese font when switching to Chinese language
        if (index == RG_LANG_ZH && gui.font_index < RG_FONT_LXGW_LITE)
        {
...
{
    const rg_font_t *font = rg_font_get_combined(index);
    if (!font)
        return false;
...
static rg_gui_event_t log_level_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    const char *log_level_names[] = {"Printf", "Error", "Warn", "Info", "Debug", "Verbose"};
    int level = rg_system_get_log_level();
    if (event == RG_DIALOG_PREV)
        level = (level > 0) ? level - 1 : RG_LOG_MAX - 1;
    else if (event == RG_DIALOG_NEXT)
        level = (level < RG_LOG_MAX - 1) ? level + 1 : 0;
    if (event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        rg_system_set_log_level(level);
    if (event == RG_DIALOG_INIT || event == RG_DIALOG_PREV || event == RG_DIALOG_NEXT)
        sprintf(option->value, "%s", log_level_names[level]);
    return RG_DIALOG_VOID;
}
...
{
    int font_count = rg_font_get_combined_count();
    if (event == RG_DIALOG_PREV && gui.font_index > 0 && rg_gui_set_font(gui.font_index - 1))
        return RG_DIALOG_REDRAW;
    if (event == RG_DIALOG_NEXT && gui.font_index < font_count - 1 && rg_gui_set_font(gui.font_index + 1))
        return RG_DIALOG_REDRAW;
...
static rg_gui_event_t border_update_cb(rg_gui_option_t *option, rg_gui_event_t event)
{
    if (event == RG_DIALOG_ENTER)
    {
        char *path = rg_gui_file_picker(_("Border"), RG_BASE_PATH_BORDERS, NULL, true);
        if (path != NULL)
        {
...
void rg_gui_debug_menu(void)
{
    ...
    char overclock[20], log_level[16];
    ...
    const rg_gui_option_t options[] = {
        ...
        {0x000, _("Overclock "), overclock,    RG_DIALOG_FLAG_NORMAL, &overclock_cb},
        RG_DIALOG_SEPARATOR,
        ...
        {0x000, _("Log level "), log_level,    RG_DIALOG_FLAG_NORMAL, &log_level_cb},
        RG_DIALOG_END
    };
    ...
    const char *log_level_names[] = {"Printf", "Error", "Warn", "Info", "Debug", "Verbose"};
    snprintf(log_level, 16, "%s", log_level_names[rg_system_get_app()->logLevel]);
    ...
    if (net.state == RG_NETWORK_DISABLED)
        snprintf(network_str, 64, "%s", _("not available"));
        ...
        snprintf(network_str, 64, "%s", _("disconnected"));
