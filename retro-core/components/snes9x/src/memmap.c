...
static void SuperFXLoROMMap(void);
...
void InitROM(bool Interleaved)
{
   ...
   else
   {
      ...
      if ((Memory.ROMType & 0xf0) == 0x10)
      {
         ...
         printf("SuperFX enabled: ROMType=%02x, ForceNoSuperFX=%d, SuperFX=%d\n", 
            Memory.ROMType, Settings.ForceNoSuperFX, Settings.SuperFX);
      }
   ...
   */
   ...
   printf("ROMType: %02x, ROMSpeed: %02x\n", Memory.ROMType, Memory.ROMSpeed);
...
static void SuperFXLoROMMap(void)
{
   ...
   printf("SuperFXLoROMMap: CalculatedSize=%lx\n", (unsigned long)Memory.CalculatedSize);
   ...
   /* Banks 40->7f and c0->ff - HiROM style mapping */
   for (c = 0; c < 0x400; c += 16)
   {
      for (i = c; i < c + 16; i++)
      {
   ...
   /* Banks 70->71: SRAM (128KB for SuperFX) */
   for (c = 0; c < 16; c++)
   {
      Memory.Map [c + 0x700] = Memory.SRAM;
      Memory.Map [c + 0x710] = Memory.SRAM + 0x10000;
      Memory.MapInfo[c + 0x700].Type = MAP_TYPE_RAM;
      Memory.MapInfo[c + 0x710].Type = MAP_TYPE_RAM;
   }

   /* Banks f0->f1: SRAM mirror (for GSU-1) */
   if (Memory.CalculatedSize <= 0x200000)
   {
      for (c = 0; c < 16; c++)
      {
         Memory.Map [c + 0xf00] = Memory.SRAM;
         Memory.Map [c + 0xf10] = Memory.SRAM + 0x10000;
         Memory.MapInfo[c + 0xf00].Type = MAP_TYPE_RAM;
         Memory.MapInfo[c + 0xf10].Type = MAP_TYPE_RAM;
      }
   }
...
void SDD1LoROMMap(void)
{
   ...
   /* Banks 60->7f - HiROM style mapping */
   for (c = 0; c < 0x200; c += 16)
   {
      for (i = c; i < c + 16; i++)
      {
         Memory.Map [i + 0x600] = &Memory.ROM [(c << 12) % Memory.CalculatedSize];
         Memory.MapInfo[i + 0x600].Type = MAP_TYPE_ROM;
      }
   }

   /* Banks c0->ff - HiROM style mapping (will be overwritten dynamically by SDD1 registers) */
   for (c = 0; c < 0x400; c += 16)
   {
      for (i = c; i < c + 16; i++)
      {
   ...
   /* Banks a0->bf 0x6000-0x7fff SRAM */
   for (c = 0; c < 0x20; c++)
   {
      Memory.Map [0xa00 + (c << 4) + 6] = MAP_LOROM_SRAM_OR_NONE;
      Memory.Map [0xa00 + (c << 4) + 7] = MAP_LOROM_SRAM_OR_NONE;
      Memory.MapInfo[0xa00 + (c << 4) + 6].Type = MAP_TYPE_RAM;
      Memory.MapInfo[0xa00 + (c << 4) + 7].Type = MAP_TYPE_RAM;
   }
