...
static uint8_t *sdd1_dma_source = NULL;
...
/**********************************************************************************************/
IRAM_ATTR void S9xDoDMA(uint8_t Channel)
{
   uint8_t Work;
   int32_t count;
   int32_t inc;
   ...
   if (Settings.SuperFX && (d->BAddress == 0x18 || d->BAddress == 0x19))
   {
      dma_count++;
      if (dma_count <= 5 || dma_count % 60 == 0)
      {
         printf("DMA #%lu: Ch=%d BAddr=%02x ABank=%02x AAddr=%04x count=%04x dir=%d\n",
             (unsigned long)dma_count, Channel, d->BAddress, d->ABank, d->AAddress, 
             (unsigned int)(count ? count : 0x10000), d->TransferDirection);
      }
   }
   ...
   if (Settings.SDD1)
   {
      if (d->AAddressFixed && Memory.FillRAM[0x4801] > 0)
      {
         ...
         {
            in_ptr += d->AAddress;
            SDD1_decompress(sdd1_decode_buffer, in_ptr, count);
            sdd1_dma_source = sdd1_decode_buffer;
            inc = 1;
         }
      }
...
IRAM_ATTR void S9xStartHDMA(void)
{
   uint8_t i;
