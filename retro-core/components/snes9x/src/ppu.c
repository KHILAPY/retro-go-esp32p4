...
IRAM_ATTR void S9xLatchCounters(bool force)
{
...
IRAM_ATTR void S9xUpdateHTimer()
{
...
IRAM_ATTR void S9xFixColourBrightness()
{
...
void S9xSetPPU(uint8_t Byte, uint16_t Address)
{
      ...
      case 0x2100: /* Brightness and screen blank bit */
         {
            ...
            total_writes++;
            if (total_writes <= 20 || (Byte & 0x80) != (Memory.FillRAM[0x2100] & 0x80))
            {
               printf("REG2100 #%lu: val=%02x old=%02x brightness=%d forcedBlank=%d->%d\n",
                      (unsigned long)total_writes, Byte, Memory.FillRAM[0x2100], Byte & 0xF,
                      (Memory.FillRAM[0x2100] >> 7) & 1, (Byte >> 7) & 1);
            }
            if (Byte != Memory.FillRAM [0x2100])
            {
               bool forced_blank_changing = ((Memory.FillRAM[0x2100] & 0x80) != (Byte & 0x80));
               
               if (forced_blank_changing && (Byte & 0x80))
               {
                  printf("REG2100: Temporarily setting ForcedBlanking=0 before FLUSH_REDRAW\n");
   ...
   else
   {
      ...
      else if (Settings.SuperFX && Address >= 0x3000 && Address <= 0x32ff)
