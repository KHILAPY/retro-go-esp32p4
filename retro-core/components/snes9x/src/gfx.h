...
typedef struct ESP_CACHE_ALIGNED_ATTR
{
   uint8_t*    Screen;
   uint8_t*    SubScreen;
   uint8_t*    ZBuffer;
...
static IRAM_ATTR INLINE uint16_t COLOR_ADD(uint16_t C1, uint16_t C2)
{
    return COLOR_ADD_OPT(C1, C2);
}

static IRAM_ATTR INLINE uint16_t COLOR_SUB(uint16_t C1, uint16_t C2)
{
    return COLOR_SUB_OPT(C1, C2);
}
