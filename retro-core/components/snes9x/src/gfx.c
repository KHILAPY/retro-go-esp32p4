...
IRAM_ATTR void S9xStartScreenRefresh(void)
{
...
IRAM_ATTR void RenderLine(uint8_t C)
{
   ...
   line_count++;
   if (line_count <= 5 || line_count % 60 == 0)
   {
      printf("RenderLine #%lu: C=%d RenderThisFrame=%d CurrentLine=%ld ForcedBlanking=%d\n",
             (unsigned long)line_count, C, IPPU.RenderThisFrame, (long)IPPU.CurrentLine, PPU.ForcedBlanking);
   }
...
IRAM_ATTR void S9xEndScreenRefresh(void)
{
   if (IPPU.RenderThisFrame)
   {
      static uint32_t end_count = 0;
      bool saved_forced_blank = PPU.ForcedBlanking;
      
      PPU.ForcedBlanking = false;
      
      end_count++;
      if (end_count <= 5 || end_count % 60 == 0)
      {
         printf("S9xEndScreenRefresh #%lu: PreviousLine=%ld CurrentLine=%ld ForcedBlanking=%d (saved=%d)\n",
                (unsigned long)end_count, (long)IPPU.PreviousLine, (long)IPPU.CurrentLine, PPU.ForcedBlanking, saved_forced_blank);
      }
      FLUSH_REDRAW();
      ...
      PPU.ForcedBlanking = saved_forced_blank;
   }
...
void S9xUpdateScreen(void)
{
   ...
   else
   {
      ...
      else
      {
         ...
         {
            printf("RenderScreen SKIPPED #%lu: ForcedBlanking=1, starty=%lu endy=%lu\n",
                   (unsigned long)skip_count, (unsigned long)starty, (unsigned long)endy);
         }
         for (y = starty; y <= endy; y++)
            memset(GFX.ZBuffer + y * GFX.ZPitch, 0, IPPU.RenderedScreenWidth);
         GFX.DB = GFX.ZBuffer;
...
// SuperFX屏幕渲染函数
// 将SuperFX SRAM中的像素数据复制到主屏幕缓冲区
void S9xRenderSuperFXScreen(void)
{
   uint32_t y, x;
   uint16_t *dst_line;
   uint32_t mode;
   ...
   mode = GSU.vMode;
   screen_height = GSU.vScreenHeight;
   ...
   if (GSU.apvScreen[0] == NULL)
   {
      return;
   }

   for (y = 0; y < screen_height && y < SNES_HEIGHT; y++)
   {
      dst_line = (uint16_t*)(GFX.Screen + y * GFX.Pitch2);
         ...
         case 0:
            {
               ...
               for (x = 0; x < 256; x++)
               {
                  plane0 = GSU.apvScreen[y >> 3] + GSU.x[x >> 3] + ((y & 7) << 1);
                  plane1 = plane0 + 1;
                  ...
                  dst_line[x] = IPPU.ScreenColors[color_index];
               }
            }
            break;
         ...
         case 3:
            {
               ...
               for (x = 0; x < 256; x++)
               {
                  src = GSU.apvScreen[y >> 3] + GSU.x[x >> 3] + ((y & 7) << 1);
                  color_index = src[(x & 7) ^ 7];
