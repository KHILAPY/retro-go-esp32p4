...
GBA_CACHE_ALIGN u32 reg[64];
GBA_CACHE_ALIGN u32 spsr[6];
GBA_CACHE_ALIGN u32 reg_mode[7][7];

GBA_CACHE_ALIGN u16 oam_ram[512];
GBA_CACHE_ALIGN u16 palette_ram[512];
GBA_CACHE_ALIGN u16 palette_ram_converted[512];
#ifndef RETRO_GO
u8 ewram[1024 * 256 * 2];
u8 iwram[1024 * 32 * 2];
u8 vram[1024 * 96];
...
#endif
GBA_CACHE_ALIGN u16 io_registers[512];
#endif
...
arm_loop:
       ...
       if(likely(condition == 0x0E))
       {
          /* AL (Always) - 最常见情况，直接执行 */
       }
...
skip_instruction:
    ...
    } while(likely(cycles_remaining > 0));
