...
GBA_CACHE_ALIGN u16* gba_screen_pixels = NULL;
...
// ============================================================================
// RGB565 Pixel Optimization Functions (ESP32-P4)
// ============================================================================

// RGB565 Alpha Blend: result = src * alpha + dst * (32 - alpha)
// alpha range: 0-32 (5-bit precision for GBA blend)
static GBA_ALWAYS_INLINE u16 gba_pixel_blend565(u16 src, u16 dst, u32 alpha)
{
  u32 inv_alpha = 32 - alpha;
  
  u32 src_r = (src >> 11) & 0x1F;
  u32 src_g = (src >> 5) & 0x3F;
  u32 src_b = src & 0x1F;
  
  u32 dst_r = (dst >> 11) & 0x1F;
  u32 dst_g = (dst >> 5) & 0x3F;
  u32 dst_b = dst & 0x1F;
  
  u32 r = (src_r * alpha + dst_r * inv_alpha) >> 5;
  u32 g = (src_g * alpha + dst_g * inv_alpha) >> 6;
  u32 b = (src_b * alpha + dst_b * inv_alpha) >> 5;
  
  return (r << 11) | (g << 5) | b;
}

// RGB565 Add with saturation: result = min(31, src_r + dst_r), etc.
static GBA_ALWAYS_INLINE u16 gba_pixel_add565(u16 src, u16 dst)
{
  u32 src_r = (src >> 11) & 0x1F;
  u32 src_g = (src >> 5) & 0x3F;
  u32 src_b = src & 0x1F;
...
// RGB565 Subtract with saturation: result = max(0, dst_r - src_r), etc.
static GBA_ALWAYS_INLINE u16 gba_pixel_sub565(u16 src, u16 dst)
{
  s32 src_r = (src >> 11) & 0x1F;
  s32 src_g = (src >> 5) & 0x3F;
  s32 src_b = src & 0x1F;
  
  s32 dst_r = (dst >> 11) & 0x1F;
  s32 dst_g = (dst >> 5) & 0x3F;
  s32 dst_b = dst & 0x1F;
...
// RGB565 Brightness: result = pixel * brightness / 16
// brightness range: 0-16 (GBA BLDY register)
static GBA_ALWAYS_INLINE u16 gba_pixel_brightness565(u16 pixel, u32 brightness)
{
  u32 r = ((pixel >> 11) & 0x1F) * brightness >> 4;
  u32 g = ((pixel >> 5) & 0x3F) * brightness >> 4;
  u32 b = (pixel & 0x1F) * brightness >> 4;
  
  return (r << 11) | (g << 5) | b;
}

// RGB565 Darken: result = pixel * (16 - brightness) / 16
static GBA_ALWAYS_INLINE u16 gba_pixel_darken565(u16 pixel, u32 brightness)
{
  u32 inv = 16 - brightness;
  u32 r = ((pixel >> 11) & 0x1F) * inv >> 4;
  u32 g = ((pixel >> 5) & 0x3F) * inv >> 4;
  u32 b = (pixel & 0x1F) * inv >> 4;
  
  return (r << 11) | (g << 5) | b;
}

// RGB565 Brighten (towards white): result = pixel + (white - pixel) * brightness / 16
static GBA_ALWAYS_INLINE u16 gba_pixel_brighten565(u16 pixel, u32 brightness)
{
  u32 r = ((pixel >> 11) & 0x1F);
  u32 g = ((pixel >> 5) & 0x3F);
  u32 b = (pixel & 0x1F);
  
  r = r + ((31 - r) * brightness >> 4);
  g = g + ((63 - g) * brightness >> 4);
  b = b + ((31 - b) * brightness >> 4);
...
GBA_CACHE_ALIGN s32 affine_reference_x[2];
GBA_CACHE_ALIGN s32 affine_reference_y[2];
  ...
  if (can_saturate) {
    ...
    while (start < end) {
      ...
      if ((st_objs && force_blend) || (do_blend && bldtype == BLEND_ONLY)) {
        ...
        if (bldtype == BLEND_DARK)
          dst[start++] = gba_pixel_darken565(pidx, brightf);
        else
          dst[start++] = gba_pixel_brighten565(pidx, brightf);
      }
      else {
...
static void merge_brightness(u32 start, u32 end, u16 *srcdst) {
  ...
  while (start < end) {
    ...
    if ((spix & 0x200) == 0x200) {
      if (bldtype == BLEND_DARK)
        pixcol = gba_pixel_darken565(pixcol, brightness);
      else
        pixcol = gba_pixel_brighten565(pixcol, brightness);
    }
...
static void render_backdrop(u32 start, u32 end, u16 *scanline) {
  ...
  if (bd_1st_target && (effect == COL_EFFECT_BRIGHT || effect == COL_EFFECT_DARK)) {
    u32 brightness = MIN(16, read_ioreg(REG_BLDY) & 0x1F);
    if (effect == COL_EFFECT_BRIGHT)
      pixcol = gba_pixel_brighten565(pixcol, brightness);
    else
      pixcol = gba_pixel_darken565(pixcol, brightness);
  }
...
IRAM_ATTR void update_scanline(void)
{
