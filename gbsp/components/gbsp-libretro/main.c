...
GBA_CACHE_ALIGN u32 cpu_ticks = 0;
GBA_CACHE_ALIGN u32 execute_cycles = 0;
GBA_CACHE_ALIGN s32 video_count = 0;
...
IRAM_ATTR u32 function_cc update_gba(int remaining_cycles)
{
  u32 changed_pc = 0;
  ...
  do
  {
    ...
    if (unlikely(check_and_raise_interrupts()))
    ...
    if (unlikely(reg[CPU_HALT_STATE] == CPU_DMA)) {
  ...
  } while(unlikely(reg[CPU_HALT_STATE] != CPU_ACTIVE) && likely(!frame_complete));
