...
DEFAULT_TARGET = os.getenv("RG_TOOL_TARGET", "esp32p4")
...
parser.add_argument(
    "--no-networking", action="store_true", default=DEFAULT_NO_NETWORKING, help="Build without networking support"
)
parser.add_argument(
    "--with-networking", action="store_true", help="Build with networking support"
)
parser.add_argument(
...
if os.path.exists(f"components/retro-go/targets/{args.target}/env.py"):
    with open(f"components/retro-go/targets/{args.target}/env.py", "rb") as f:
        ...
        env_globals = {"IDF_TARGET": IDF_TARGET, "no_networking": False}
        exec(f.read(), env_globals)
        IDF_TARGET = env_globals.get("IDF_TARGET", IDF_TARGET)
        env_no_networking = env_globals.get("no_networking", False)
...
# Handle networking flags: --with-networking overrides --no-networking
no_networking = args.no_networking
if args.with_networking:
    no_networking = False

# Ensure the target is set correctly before building
for app in apps:
...
try:
    ...
    if command in ["build", "build-fw", "build-img", "release", "run", "profile", "install"]:
        print("=== Step: Building ===\n")
        for app in apps:
            build_app(app, args.target, command == "profile", no_networking, command == "release")
